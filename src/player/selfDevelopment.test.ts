...
describe("self-development domain", () => {
  ...
  it("charges one consumable instructor hint through additional dark-psychology study", () => {
    const runtime = testRuntime();
    const service = selfDevelopmentSystem(runtime);
    const state = stateAtNight(runtime);

    const result = service.performActivity(state, "dark_psychology", 1);
    expect(result).toMatchObject({
      activityId: "dark_psychology",
      fatigueDelta: 2,
      hintChargeDelta: 1,
    });
    expect(service.hintCharges(state)).toBe(1);
    expect(service.consumeHint(state)).toBe(true);
    expect(service.hintCharges(state)).toBe(0);
    expect(service.consumeHint(state)).toBe(false);
  });
