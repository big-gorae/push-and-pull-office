...
- 흰 배경 파생본: `v2-love-office-style-tan-white-bg-seed-2999493819.png`
- 상태: 사용자가 현재 강사 v2 원화로 채택했으며, 흰 배경 파생본을 실제 사용 기준 이미지로 삼는다. 원본과 파생본 모두 삭제하거나 덮어쓰지 않는다.
- 생성 방식: Opus 표준 `Generate 1 Image · 0 Anlas` 단일 생성.
...
현재 강사 v2 채택본은 seed 2999493819의 흰 배경 파생본이다. 1차는 얼굴 골격과 전체 인상, seed 505059432는 비미남형 매부리코, 목 비례·코잔등 보정본은 정상 목, 3차는 올블랙 의상 참고 후보로 각각 보존한다. 채택본 역시 후속 보정 시 덮어쓰지 않고 새 파일로 파생한다.
