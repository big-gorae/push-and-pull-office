...
`beautiful`, `gorgeous`, `exceptionally beautiful face` 같은 주관적 문구는 캐릭터 미형 앵커로 쓰지 않는다. V4.5 Full의 Quality Tags가 `very aesthetic`, `masterpiece` 같은 공식 품질 태그를 자동 적용하고, 인물의 생김새는 `long eyelashes`, `small nose`, `soft full lips`와 캐릭터별 눈매·얼굴형·턱선처럼 눈에 보이는 형태로 지정한다.

일반 여성 캐릭터는 `narrow waist`, `1.2::large breasts::`, `breasts`를 사용한다. 강유진만 의도적으로 비현실적인 만화 체형을 갖도록 `curvy`, 강화된 `narrow waist`, `wide hips`, `long legs`, `slim legs`, 강화된 `huge breasts`를 사용한다. 정본의 164cm 키는 바꾸지 않고 신체 내부 비율만 과장하며, 허리선이 보이도록 기존 셔츠와 와이드 팬츠의 종류·색을 유지한 채 fitted/high-waisted 속성만 더한다. 모든 여성 캐릭터의 UC에는 `flat chest`, `small breasts`를 유지한다.

윤서아의 큰 눈은 정체성 태그로 유지하고, 미소가 있는 상황에서만 올라간 볼, 굽은 아래 눈꺼풀, 반달형 눈웃음, 살짝 비대칭인 입꼬리와 은은한 잇몸 미소를 조합한다. 차민경은 눈 바깥꼬리 아래의 `mole under eye`와 목 아래·쇄골 바로 밑 상부 가슴의 `mole on collarbone`을 각각 가중 태그와 짧은 위치 문장으로 고정한다. 두 점을 함께 요구하므로 `multiple moles`를 UC에 넣지 않는다. 작은 점이 실제 결과에서 보이도록 기본 상황은 `character_upper_body` 구도를 사용한다. 차민경의 냉미녀 인상은 찡그림·좁힌 눈·한쪽 눈썹이 아니라 작은 타원형 얼굴, 섬세한 턱선, 큰 아몬드형 눈과 정돈된 무표정으로 만든다. 눈에는 동공 효과를 더 붙이지 않고 Quality Tags와 `game cg, year 2024`의 기본 미형을 사용한다. 공통 UC는 큰 가슴 자체를 막지 않으며 실제 노출과 홍채색 혼합만 막는다.
