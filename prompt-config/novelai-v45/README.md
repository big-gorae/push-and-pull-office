...
공통 `styleTags`는 메인 로비의 유은솔·차민경·신나경 콘셉트 아트를 기준으로 한다. 검증 태그인 `visual novel`, `year 2024`와, 자연어 작화 지시인 상업 미연시 게임 CG 마감, 부드러운 셀 음영, 선명한 눈, 섬세한 얼굴 비례, 따뜻한 발광 색감을 공통 앵커로 사용한다. 숫자 강조 문법, HDR, 시네마틱·망가 하이브리드, 러프 스케치 프리셋은 사용하지 않는다. 참조 프롬프트의 머리색, 홍채색, 직업, 의상, 배경, 포즈와 카메라 각도도 캐릭터·장면 내용이므로 공통 작화에 넣지 않는다.
