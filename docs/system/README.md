# [구현] 시스템 설계 문서 허브

> 마지막 정리: 2026-08-13
> 이 문서는 Love Office의 시스템 설계 문서를 찾고 상태를 판정하는 단일 진입점이다.

## 관리 원칙

1. 신규 시스템 설계는 `docs/system/`에 작성한다.
2. 모든 시스템 설계 문서의 첫 제목은 다음 상태 중 하나로 시작한다.
   - `[구현]`: 현재 런타임·플레이어·에디터에 적용되어 있다.
   - `[부분 구현]`: 기반은 적용됐지만 문서의 핵심 인수 조건 일부가 남아 있다.
   - `[미구현]`: 합의된 설계 또는 제안이며 실행 코드에는 아직 적용되지 않았다.
   - `[폐기]`: 더 이상 구현하지 않으며 다른 문서의 근거로 사용하지 않는다.
3. 제목 상태만 바꾸지 않는다. 구현 상태가 변하면 문서의 현황, 구현 근거, 남은 작업과 이 색인을 함께 갱신한다.
4. 사람의 제품 의도는 `docs/`, 기계가 검증하는 계약은 `story/`, 실제 동작은 `src/`와 `tools/`가 권위 원본이다.
5. 문서와 코드가 다르면 현재 구현 설명에는 코드와 생성 런타임을 우선하고, 문서는 차이를 명시한 뒤 수정한다.
6. 스토리 장면의 사실과 대사는 시스템 문서에 복제하지 않고 안정 ID와 원본 문서만 참조한다.

캐릭터 상호작용 중 반응 화법 축의 내부 명칭은 **MBTI 요소**다. `interaction.target/support_styles`는 실제 반응 상대와 행동으로 관찰되는 지원 화법이고, `push_pull.target`은 서정우의 리듬 계산 대상이다. 두 필드는 서로를 대신하지 않으며, 명시적인 요청·거절에서는 `literal_respect`와 장면의 현실이 평상시 선호보다 우선한다. 선택지·대사·상시 HUD에는 MBTI/F/T 명칭을 노출하지 않고, 캐릭터 탭의 해금형 프로필에서만 유형을 표시한다. 기계 계약은 `story/SPEC.md`, `story/AI_AUTHORING_RULES.md`와 하네스가 소유한다.

## 현재 자동화 운영 상태

2026-08-02 현재 다음 자동 실행은 일시 중지되어 있다.

- 로컬 pre-push 전체 검증
- GitHub `Love Office CI`
- `main` 필수 `required` 체크와 자동 병합
- Dependabot 보안 업데이트와 정기 버전 업데이트

설계와 검사 코드는 삭제하지 않았다. 필요할 때 `npm run verify`로 수동 일괄 검증할 수 있으며, 자동화를 다시 켤 때는 로컬 훅·GitHub 워크플로·브랜치 규칙·Dependabot 제한을 함께 복원한다.

## 핵심 계약

이 파일들은 시스템 설명보다 강한 기계 계약 또는 제작 계약이다. 구조상 `story/`에 남겨 둔다.

| 상태 | 문서 | 책임 |
|---|---|---|
| 구현 | [스토리 제작 시스템](../../story/README.md) | 제작 원본, 빌드 산출물과 도구 사용법 |
| 구현 | [스토리 데이터 명세](../../story/SPEC.md) | 상태, 캠페인, 이벤트와 단일 대사·연출 장면 계약 |
| 구현 | [게임 런타임 통합](../../story/RUNTIME_INTEGRATION.md) | 런타임·세이브·백로그 연동 |
| 구현 | [요구사항 검증표](../../story/VERIFICATION.md) | validator와 테스트가 증명해야 할 항목 |
| 구현 | [AI 스토리 작성 계약](../../story/AI_AUTHORING_RULES.md) | 에이전트의 제한된 장면 작성 규칙 |
| 구현 | [회사 월드 바이블](../../story/world/README.md) | 회사·팀·직급·구성원·회의 사실 |

## 기존 시스템 설계

시스템 설계 문서는 이 디렉터리에 모으고 이 표가 위치와 구현 상태를 통합 관리한다. 다른 기획 문서에서는 `docs/system/`의 현재 경로를 참조한다.

| 상태 | 문서 | 현재 구현 근거 또는 남은 범위 |
|---|---|---|
| 구현 | [밀당 시스템 설계](push-pull-system.md) | `src/pushPull.ts`, MBTI 요소 데이터인 `interaction_preferences`, 선택지 `interaction`, 독립된 `push_pull.target`, 하네스·플레이어 대상 회귀 테스트와 2일차 수직 슬라이스 |
| 구현 | [밀당 UI/UX 설계](push-pull-ui-ux.md) | 리듬 막대, 선택 화면과 디버그 표시 |
| 구현 | [웹 미연시 플레이어 설계](web-game-player-design.md) | 타임라인 런타임, 저장·불러오기, 백로그, 오토·스킵 |
| 구현 | [다국어·상황별 대사 관리](localization-context-dialogue-redesign.md) | 단일 레지스트리, fallback, variant와 v3 ID 세이브 |
| 부분 구현 | [Tauri 스토리 에디터 설계](tauri-story-editor-design.md) | 구조화 편집과 안전 저장은 구현, 일부 최종 인수 조건은 잔존 |
| 구현 | [인게임 대사 편집 모드](ingame-dialogue-editing.md) | Tauri에서 원문·번역·합성 원본을 검증 저장하고 즉시 반영하며, 정확한 source open·draft·충돌 보호 Undo 지원 |
| 구현 | [매력도와 저녁·밤 페이즈](charm-evening-night-system.md) | 핵심 자기계발 상태·밤 활동·표현 선택지 구현, 관심 메모는 후속 확장 |
| 미구현 | [생존 모드 확정안](survival-mode.md) | 해금 메타만 존재하고 별도 캠페인·주인공·장면 없음 |

## 신규 개선 설계

| 상태 | 이전 제안 번호 | 문서 | 핵심 목적 |
|---|---:|---|---|
| 미구현 | - | [에디터 비차단 저장 아키텍처](editor-nonblocking-save-architecture.md) | 입력과 영속화를 분리한 공통 자동 저장, 수동 flush, 버전 승인과 증분 백엔드 |
| 구현 | 1 | [게임 모드·캠페인 분리](game-mode-campaign-separation.md) | 스토리와 어나더 스토리의 캠페인·연속성 분리; 장면 레이어 시스템은 폐기 |
| 부분 구현 | 3 | [MBTI 요소·밀당 학습 피드백](push-pull-onboarding-feedback.md) | MBTI 요소용 지원 화법·두 대상·2일차 수직 슬라이스는 구현, 의미 이벤트·학습 플래그·전용 연출과 접근성은 남음 |
| 미구현 | 4 | [엔딩·회상 갤러리](ending-memory-gallery.md) | 엔딩 수집과 단일 장면 재생; 과거 두 레이어 비교안은 폐기 |
| 미구현 | 5 | [세이브 호환·내보내기](save-versioning-export.md) | 스토리 개정 중 세이브 보호와 로컬 백업 |

## 상태 변경 절차

시스템을 구현하기 시작할 때:

1. 문서의 구현 범위와 제외 범위를 먼저 고정한다.
2. 관련 데이터 스키마와 세이브 마이그레이션을 코드보다 먼저 확정한다.
3. 최소 수직 슬라이스와 인수 테스트를 구현한다.
4. 문서의 인수 조건을 모두 만족하면 제목과 이 색인을 `[구현]`으로 변경한다.
5. 일부만 배포하면 `[부분 구현]`을 유지하고 남은 항목을 체크리스트로 남긴다.

문서 상태는 기능의 희망 상태가 아니라 현재 `main`에서 확인 가능한 상태를 나타낸다.
