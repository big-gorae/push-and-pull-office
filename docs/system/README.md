...
캐릭터 상호작용은 두 축으로 관리한다. `interaction.target/support_styles`는 실제 반응 상대와 행동으로 관찰되는 지원 화법이고, `push_pull.target`은 한도윤의 리듬 계산 대상이다. 두 필드는 서로를 대신하지 않으며, 명시적인 요청·거절에서는 `literal_respect`와 장면의 현실이 평상시 선호보다 우선한다. 기계 계약은 `story/SPEC.md`, `story/AI_AUTHORING_RULES.md`와 하네스가 소유한다.
