...
Replika가 보여 주는 핵심은 “사용자가 좋아하는 답변”과 “캐릭터가 원래 할 답변”의 긴장이다. Love Office에서는 사용자 반응이 말투 품질 개선에는 쓰일 수 있어도, 유은솔이 서정우에게 연애 감정을 느끼지 않는다는 불변 사실 같은 캐논을 바꾸어서는 안 된다.
...
2. **승인된 기준 대사 추가**
   - 캐릭터마다 최소한 안전한 동료, 상사, 서정우, 긴장, 분노/공포, 업무 집중 상황의 예시를 둔다.
...
- 기억 목록과 삭제/수정 기능 제공
- 서정우 채팅은 현재의 유해 행동 저작 금지 규칙을 별도 시스템 안전 규칙으로 강제
...
| 영역 | 검사 예시 | 출시 기준 제안 |
| --- | --- | --- |
| 불변 사실 | 유은솔이 서정우에게 연애 감정을 느낀다고 말하는가 | 중대 모순 0건 |
