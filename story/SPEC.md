...
`survivor_view`는 유저에게 `어나더 스토리`로 표시하는 안정 ID다. 최종 선택 가능한 윤서아 또는 강유진 라우트의 첫 엔딩을 보면 해금한다. 현재 강유진 라우트는 미작성 상태이므로 실제 플레이에서는 윤서아 라우트만 이 조건을 충족한다. 안내 문구는 `새로운 그녀로 새로운 이야기를 만들어 보아요`로 고정한다. `survivor_view` 캠페인의 플레이어 캐릭터는 후속 결정 전까지 스토리 데이터에 고정하지 않는다.
...
스토리 모드의 표면 공략 후보는 윤서아·차민경·강유진 세 명이다. 실제 최종 선택 후보는 윤서아와 강유진이며, 차민경은 호감도·밀당·개별 사건을 갖지만 결정적 선택에서 제외되는 위장 공략이다. 강유진 전용 라우트·고백·엔딩은 아직 작성되지 않았다. 강유진은 사실관계를 확인하고 이상 징후를 알아차릴 수 있지만, 기본 결말에서 갑자기 모든 증거를 완성하거나 피해자를 구해 파국을 취소하는 해결사로 기능해서는 안 된다. 생존 모드에서의 플레이어·조력자·조연 역할은 별도로 결정한다.
...
- 라우트의 `final_selectable`은 엔딩 도달 시 실제 최종 공략 완료로 기록할 수 있는지를 선언한다. 위장 공략인 차민경 라우트는 `false`이며 엔딩에 도달해도 `progress.cleared_routes`나 모드 해금을 추가하지 않는다.
