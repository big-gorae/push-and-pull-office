...
- `common.day_02_practical_meeting`: 서아의 감정 인정·선택권과 민경의 사실 확인·복구안을 비교하는 네 선택 수직 슬라이스
...
- 최종 선택 가능한 서아 또는 민경 라우트를 최초 클리어하면 생존 모드가 열리고, 폐기된 붕괴 모드는 열리지 않음
- 생존 모드가 독립된 평행세계로 빌드되고 스토리 모드 결말을 보존함
- 강유진이 최종 선택 불가능한 표면 공략 후보로 일반 캠페인 레인과 초기 상태에 존재하고 결말 해결사 역할을 갖지 않음
- 강유진 호감도가 80에서 멈추고 기존 초과 저장값도 80으로 보정되지만 콤보와 숨은 현실 효과는 계속 누적됨
...
- 공용 2일차 장면에서 민경 대상 선택은 서아가 아니라 민경의 호감도와 콤보 대상만 갱신함
