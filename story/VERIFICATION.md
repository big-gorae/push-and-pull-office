...
- 최종 선택 가능한 서아 또는 유진 라우트를 최초 클리어하면 생존 모드가 열리고, 위장 공략인 민경 엔딩으로는 열리지 않으며 폐기된 붕괴 모드도 열리지 않음
- 생존 모드가 독립된 평행세계로 빌드되고 스토리 모드 결말을 보존함
- 강유진이 표면 공략 후보이자 최종 선택 후보로 일반 캠페인 레인과 초기 상태에 존재하지만, 아직 전용 라우트가 없고 결말 해결사 역할도 갖지 않음
