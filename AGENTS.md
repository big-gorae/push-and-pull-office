...
- Treat `켜줘`, `다시`, and `다시 켜줘` as focus/reuse requests, not restart requests. If any healthy Authoring Build `main` or `authoring-play` window exists, consider the launch successful even when a separate blank or stale editor window is also visible; never start another instance because of that extra window.
